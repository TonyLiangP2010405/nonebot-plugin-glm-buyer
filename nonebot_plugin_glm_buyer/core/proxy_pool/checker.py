...
from nonebot_plugin_glm_buyer.core.proxy_pool.service import (
    RuntimeProxyConfig,
    check_proxy_health,
    fetch_proxy_sources,
...
def main() -> int:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=config.health_check_concurrency) as executor:
        future_map = {
            executor.submit(check_proxy_health, proxy, config, strict_mode=strict_mode): proxy for proxy in proxies
        }
