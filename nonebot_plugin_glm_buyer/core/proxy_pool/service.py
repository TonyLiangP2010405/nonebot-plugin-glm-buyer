...
class ProxyPoolConfig:
    source_base_dir: Path = field(default_factory=Path.cwd)
    ...
    def update(self, proxies: list[UpstreamProxy], *, fast_window: int) -> None:
        ...
        with self._lock:
            ...
            self._cooldown_until = {
                key: until for key, until in self._cooldown_until.items() if key in valid_keys and until > now
            }
            self._index = 0
            logger.info(
...
class BuiltinProxyPoolService:
    ...
    def start(self) -> None:
        ...
        self._servers = [
            self._make_server(
                "socks5-strict", self.config.ports.socks5_strict, "socks5", "STRICT", self.strict_pool, timeout
            ),
            self._make_server(
                "socks5-relaxed", self.config.ports.socks5_relaxed, "socks5", "RELAXED", self.relaxed_pool, timeout
            ),
            self._make_server(
                "http-strict", self.config.ports.http_strict, "http", "STRICT", self.strict_pool, timeout
            ),
            self._make_server(
                "http-relaxed", self.config.ports.http_relaxed, "http", "RELAXED", self.relaxed_pool, timeout
            ),
        ]
        try:
    ...
    def refresh_once(self) -> None:
        ...
        try:
            logger.info(
                "Refreshing proxy pool from %d regular sources and %d special sources",
                len(self.config.proxy_list_urls),
                len(self.config.special_proxy_list_urls),
            )
...
def resolve_config_path() -> Path:
    configured = getattr(get_settings(), "proxy_pool_config_path", None)
    if configured is not None:
        return Path(configured)
    raise FileNotFoundError("proxy pool config path is not configured")
...
def read_proxy_source(source: str, *, base_dir: Path | None = None) -> str:
    if base_dir is None:
        base_dir = Path.cwd()
    source = source.strip()
...
def http_proxy_connect(
    ...
    *,
    ...
    try:
        ...
        if proxy.username:
            token = base64.b64encode(f"{proxy.username}:{proxy.password}".encode()).decode("ascii")
...
def encode_socks5_address(host: str) -> bytes:
    try:
        ...
        if len(encoded) > 255:
            raise RuntimeError("SOCKS5 target host is too long") from None
