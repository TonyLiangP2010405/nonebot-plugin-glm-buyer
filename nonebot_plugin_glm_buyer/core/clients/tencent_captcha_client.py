...
from nonebot_plugin_glm_buyer.core.browser_profiles import resolve_user_agent
from nonebot_plugin_glm_buyer.core.clients.fingerprint_http import FingerprintHttpClient
from nonebot_plugin_glm_buyer.core.config import Settings, get_settings
from nonebot_plugin_glm_buyer.core.errors import UpstreamRequestError
from nonebot_plugin_glm_buyer.core.models import AccountRecord
...
class TencentCaptchaClient:
    ...
    def prehandle(self, account: AccountRecord) -> CaptchaChallenge:
        ...
        image_path = str(
            (((payload.get("data") or {}).get("dyn_show_info") or {}).get("bg_elem_cfg") or {}).get("img_url") or ""
        )
    ...
    def verify(self, account: AccountRecord, payload: dict[str, Any]) -> CaptchaVerifyResult:
        user_agent = resolve_user_agent(account.user_agent, account.browser_impersonate)
        form_body = {str(key): str(value) for key, value in payload.items() if value is not None and str(value) != ""}
