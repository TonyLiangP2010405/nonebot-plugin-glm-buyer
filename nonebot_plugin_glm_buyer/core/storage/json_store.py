    ...
    def _replace_with_retry(self, temp_path: Path) -> None:
        ...
        for delay in (*self._replace_retry_delays, 0):
            if not temp_path.exists():
                raise FileNotFoundError(f"atomic temp file vanished before replace for {self.path}")
            try:
        ...
        if last_error is not None:
            raise PermissionError(f"{last_error}. atomic replace failed after retries for {self.path}") from last_error
