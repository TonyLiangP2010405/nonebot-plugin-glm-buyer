...
from nonebot_plugin_glm_buyer.core.browser_profiles import random_browser_impersonate, resolve_browser_impersonate
from nonebot_plugin_glm_buyer.core.config import get_settings
from nonebot_plugin_glm_buyer.core.errors import BadRequestError, NotFoundError
...
from nonebot_plugin_glm_buyer.core.runtime_logging import get_runtime_log_service
from nonebot_plugin_glm_buyer.core.storage.json_store import JsonFileStore
...
class AccountStateService:
    ...
    def _session_store(self, account_id: str) -> JsonFileStore:
    ...
    def import_account(self, request: AccountImportRequest) -> PublicAccountRecord:
        ...
        def updater(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
            ...
            record = AccountRecord(
                ...
                preview_concurrency_time_enabled=bool(existing.get("preview_concurrency_time_enabled"))
                if existing
                else False,
                ...
                scheduled_start_time=str(existing.get("scheduled_start_time") or DEFAULT_SCHEDULED_START_TIME)
                if existing
                else DEFAULT_SCHEDULED_START_TIME,
    ...
    def _should_skip_today_after_schedule_update(
        ...
        *,
        ...
        time_changed = request.scheduled_start_time is not None and account.scheduled_start_time != (
            previous_scheduled_start_time or ""
        )
    ...
    def load_session(self, account_id: str) -> AccountSessionState:
        store = self._session_store(account_id)
    ...
    def save_session(self, session: AccountSessionState) -> AccountSessionState:
        ...
        store = self._session_store(session.account_id)
