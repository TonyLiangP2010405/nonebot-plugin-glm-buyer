...
from nonebot_plugin_glm_buyer.core.errors import AegisFlowError
from nonebot_plugin_glm_buyer.core.runtime_logging import get_runtime_log_service
from nonebot_plugin_glm_buyer.core.services.account_state import get_account_state_service, utc_now_iso
from nonebot_plugin_glm_buyer.core.services.payment_service import RunPausedError, get_payment_service
...
class SchedulerService:
    ...
    def __init__(self) -> None:
        ...
        self._completed_tasks: list[tuple[str, str, str]] = []  # (account_id, biz_id, qr_base64)
    ...
    def _clear_stale_schedule_statuses(self) -> None:
        ...
        for public_account in self.state_service.list_accounts():
            ...
            account.last_scheduled_run_key = ""
            account.last_scheduled_run_at = None
        ...
        try:
            ...
            if source == "scheduled":
                ...
                account.last_scheduled_run_key = scheduled_run_key or self._scheduled_run_key(
                    datetime.now(SCHEDULER_TZ).strftime("%Y-%m-%d")
    ...
    def pop_completed_tasks(self) -> list[tuple[str, str, str]]:
        """Return and clear the list of completed scheduled tasks."""
        with self._lock:
            tasks = list(self._completed_tasks)
    ...
    def _run_account_flow(self, account_id: str, source: str) -> None:
        try:
            ...
            with self._lock:
                self._completed_tasks.append((account_id, task.biz_id, task.qr_base64))
            account = self.state_service.get_account(account_id)
            account.last_schedule_status = "success"
            if source == "probe":
