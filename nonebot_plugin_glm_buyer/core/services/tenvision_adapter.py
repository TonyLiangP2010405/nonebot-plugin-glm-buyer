...
def ocr_items(result, x_offset=0, y_offset=0):
    ...
    for index, (poly, text, score) in enumerate(zip(boxes, txts, scores, strict=False)):
...
def is_ignored_item(item, prompt_item, image_h, image_w):
    ...
    return y < int(image_h * 0.22) and w > int(image_w * 0.18) and "请" not in text
...
def build_color_masks(crop):
    ...
    yellow_mask = ((hsv[:, :, 0] >= 8) & (hsv[:, :, 0] <= 55) & (hsv[:, :, 1] >= 35) & (hsv[:, :, 2] >= 60)).astype(
        np.uint8
    ) * 255
...
def split_boxes_are_reasonable(parent_bbox, boxes, expected_count):
    ...
    if not areas:
        return False
    return not min(areas) * 16 < max(areas)
...
def split_bbox_with_yellow_segments(crop, origin_x, origin_y, expected_count, horizontal):
    ...
    for idx in range(expected_count):
        ...
        part = crop[:, start:end] if horizontal else crop[start:end, :]
...
def repair_candidates_by_prompt(candidates, prompt_chars):
    ...
    unknown_candidates = [
        candidate for candidate in candidates if id(candidate) not in used_ids and not candidate.get("char")
    ]
...
def split_ocr_item(image, item, prompt_chars=None, engine=None):
    ...
    split_items = []
    for index, (char, box) in enumerate(zip(text_chars, ordered, strict=False)):
...
def build_candidate_chars(image, items, prompt_item, image_h, image_w, prompt_chars=None, engine=None):
    ...
    candidates = [
        cand for cand in candidates if len(cand.get("char") or "") == 1 or (prompt_chars and not cand.get("char"))
    ]
...
def fill_missing_prompt_chars(candidates, prompt_chars):
    ...
    for prompt_char in prompt_chars:
        ...
        if not options:
            ...
            continue
        chosen = sorted(
            options, key=lambda item: (float(item.get("score") or 0), int(item.get("area") or 0)), reverse=True
        )[0]
        used_ids.add(id(chosen))
...
def run_pipeline_bytes(image_bytes: bytes, prompt_text: str | None = None, include_debug: bool = False):
    ...
    if len(prompt_chars) < 2:
        try:
            ...
            raise RuntimeError("未识别到验证码提示文本，当前截图疑似不是验证码弹窗。") from None

    if (
        prompt_item is not None
        and not is_prompt_like_text(prompt_item["text"])
        and len(parse_prompt_chars(prompt_item["text"])) <= 2
    ):
...
def analyze_image_bytes(
    ...
    matched_scores = [
        score for score, box in zip(result["click_scores"], result["click_boxes"], strict=False) if box is not None
    ]
