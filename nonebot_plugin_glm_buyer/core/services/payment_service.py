...
from nonebot_plugin_glm_buyer.core.clients.bigmodel_client import BigModelClient, get_bigmodel_client
from nonebot_plugin_glm_buyer.core.clients.tencent_captcha_client import (
    TencentCaptchaClient,
    get_tencent_captcha_client,
)
from nonebot_plugin_glm_buyer.core.config import get_settings
from nonebot_plugin_glm_buyer.core.errors import AegisFlowError, BadRequestError, UpstreamRequestError
...
from nonebot_plugin_glm_buyer.core.proxy_pool.service import get_builtin_proxy_pool_service, is_local_proxy_url
from nonebot_plugin_glm_buyer.core.runtime_logging import FlowRun, RuntimeLogService, get_runtime_log_service
from nonebot_plugin_glm_buyer.core.services.account_state import (
    SCHEDULE_TZ,
    AccountStateService,
...
from nonebot_plugin_glm_buyer.core.services.captcha_service import CaptchaService, get_captcha_service
from nonebot_plugin_glm_buyer.core.services.network_mode_service import get_network_mode_service
from nonebot_plugin_glm_buyer.core.services.ocr_service import OcrService, get_ocr_service
from nonebot_plugin_glm_buyer.core.services.tdc_service import TencentTdcService, get_tdc_service
...
class PaymentService:
    ...
    def _submit_captcha_verify_for_session(
        ...
        *,
        ...
        try:
            ...
            verify_ok = bool(
                verify_result.ticket and verify_result.randstr and str(verify_result.error_code or "0") in {"", "0"}
            )
            self.runtime_logs.log_event(
                flow,
    ...
    def _fill_ticket_pool(
        ...
        *,
        ...
        while unused_count < target_size:
            ...
            from nonebot_plugin_glm_buyer.core.models import TicketPoolEntry
        ...
        *,
        ...
        try:
            ...
            while True:
                ...
                captcha_request = (
                    request if preview_round == 1 else request.model_copy(update={"ticket": None, "randstr": None})
                )
                try:
                    ticket, randstr = self._resolve_or_solve_preview_captcha(
                        account_id, session, captcha_request, flow=flow
                    )
                except UpstreamRequestError as exc:
        ...
        *,
        ...
        try:
            ...
            if ticket_pool_size > 0:
                ...
                deadline_time = (
                    current_account.preview_concurrency_time if current_account.preview_concurrency_time_enabled else ""
                )
                self.runtime_logs.log_event(
                    flow,
                    stage="ticket_pool",
                    ...
                    details={
                        ...
                        "drain_mode": "parallel" if current_account.ticket_pool_drain_interval_ms <= 0 else "serial",
    ...
    def _captcha_ocr_gate(self, challenge: dict[str, Any]) -> dict[str, Any]:
        ...
        point_labels = [str(item.get("label") or "").strip() for item in points if isinstance(item, dict)]
    ...
    def _ensure_context(self, account_id: str) -> tuple[AccountRecord, AccountSessionState]:
        ...
        session = self.state_service.load_session(account_id)
        if (
            not session.customer_number
            or not (session.org_id or account.org_id)
            or not (session.project_id or account.project_id)
        ):
        ...
        *,
        ...
        if preferred_project_id:
            selected_project = next(
                (item for item in projects if self._entity_id(item, ("id", "projectId")) == preferred_project_id),
                None,
            )
